...
3. Au système d'**assigner automatiquement** un relecteur à chaque exercice
...
| **Étudiant** | Choisir son nom dans une liste préchargée (pas de mot de passe) · Marquer sa présence avec un code de session · Déposer le lien de son exercice pour une session · Remplacer le lien de son exercice tant que la relecture n'a pas commencé · Consulter la note et le commentaire reçus sur son exercice (sans connaître l'identité du relecteur) |
...
- **Assignation automatique** d'un relecteur à chaque exercice déposé, parmi les étudiants présents à la session.
...
- **Relecture** d'un exercice par le relecteur assigné : note entière de 0 à 20 et commentaire. Relecture **définitive** une fois rendue.
...
| **EF9** | Le système assigne automatiquement un relecteur à chaque exercice déposé | Quand un exercice est déposé, un relecteur est choisi au hasard parmi les étudiants présents (hors auteur), et l'exercice passe en `EN_ATTENTE` | Must |
...
| **EF12** | Le relecteur note et commente l'exercice assigné | Quand je soumets une note entière entre 0 et 20 et un commentaire, l'API renvoie `200` et l'exercice passe en `RELUE` | Must |
...
| **EF15** | L'auteur d'un exercice voit sa note et le commentaire, sans l'identité du relecteur | Quand l'auteur appelle `GET /api/etudiants/{id}/exercices`, il voit la note et le commentaire de ses exercices relus, jamais le nom du relecteur | Must |
...
| **RG6** | Le relecteur est choisi automatiquement au hasard parmi les étudiants présents à la session, à l'exclusion de l'auteur de l'exercice. L'assignation a lieu au moment du dépôt de l'exercice. | Q7 |
...
| **RG9** | Une relecture rendue est définitive : elle ne peut plus être modifiée ni resoumise. Toute resoumission renvoie `409 RELECTURE_DEJA_RENDUE`. | Q15, décision A1 |
...
| **RG14** | Si aucun étudiant présent n'est disponible pour relire un exercice au moment du dépôt, l'exercice reste `EN_ATTENTE` avec la mention « relecteur à assigner » dans le tableau. | Décision A2 |
| **RG15** | Le formateur peut réassigner manuellement le relecteur d'un exercice resté `EN_ATTENTE`. | Décision A2 |
| **RG16** | Le tableau de synthèse affiche, pour chaque étudiant d'une promotion : ses présences par session, son nombre d'exercices déposés, la moyenne de ses notes reçues, et le nombre de relectures qu'il doit encore faire. | Q16 |
...
| **B5** | Schéma versionné par Flyway ou Liquibase, migrations commitées. `ddl-auto=update` interdit hors tests | Flyway. Migrations `V1__init.sql`, `V2__...` dans `backend/src/main/resources/db/migration/`. `spring.jpa.hibernate.ddl-auto=validate` en dev et prod, `create-drop` uniquement en test |
